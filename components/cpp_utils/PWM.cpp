 ...
 *
 * A timer starts ticking up from 0 to the maximum value of the bit size.  When
 it reaches
 * the maximum value, it resets.
 *
 * In the following example, we create a signal that has a frequency of 100Hz
 and is
 ...
 * @param [in] dutyResolution The size in bits of the timer.  Allowed values are
 LEDC_TIMER_10_BIT,
 * LEDC_TIMER_11_BIT, LEDC_TIMER_12_BIT, LEDC_TIMER_13_BIT, LEDC_TIMER_14_BIT,
 LEDC_TIMER_15_BIT.
 * @param [in] timer The timer to use. A value of LEDC_TIMER0, LEDC_TIMER1,
 LEDC_TIMER2 or LEDC_TIMER3.
 * @param [in] channel The channel to use.  A value from LEDC_CHANNEL0 to
 LEDC_CHANNEL1.

 * @return N/A.
 */
PWM::PWM(int gpioNum, uint32_t frequency, ledc_timer_bit_t dutyResolution,
...
PWM::PWM(int gpioNum, ledc_channel_t channel)
    : PWM(gpioNum, 150, LEDC_TIMER_10_BIT, LEDC_TIMER_0, channel)
{}
 ...
 *
 * The duty cycle value is a numeric between 0 and the maximum bit size.  When
 * the %PWM tick value is less than this value, the output is high while when it
 * is higher than this value, the output is low.
...
{
    ESP_ERROR_CHECK(
        ::ledc_stop(LEDC_HIGH_SPEED_MODE, m_channel, idleLevel ? 1 : 0));
} // stop
